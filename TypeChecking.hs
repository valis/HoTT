...
typeOf'depType :: T.DBIndex -> M.Map String DBIndex -> Ctx -> [TypedVar] -> Expr -> EDocM Value
...
typeOf'depType i mctx actx@(ctx,lctx) (TypedVar _ vars t : list) e = do
    ...
    (mctx',lctx',i') <- updateCtx i mctx lctx (evalRaw i actx t (Just tv)) vars
    ev <- typeOf i' (ctx,lctx') e'
    maxType i' t e' tv ev
  where
...
typeOf :: T.DBIndex -> M.Map String DBIndex -> Ctx -> Expr -> EDocM Value
typeOf i mctx ctx e = typeOfH i mctx ctx e N

typeOfH :: T.DBIndex -> M.Map String DBIndex -> Ctx -> Expr -> H -> EDocM Value
...
typeOfH i mctx ctx (Lam _ (x:xs) e) (T r@(Spi z fv a b)) =
    let p = if null xs then getPos e else binderGetPos (head xs)
        (x',e') = renameExpr fv (unBinder x) (Lam (PLam (p,"\\")) xs e)
        v = svar x' a
    in typeOfH (i + 1) (M.insert x' i mctx) (M.insert x' (v,a) ctx) e' (T (b 0 [] v)) >> return r
typeOfH i mctx _ (Lam _ (Binder arg : _) _) (T ty) =
    let lc = case arg of
            Arg (PIdent (p,_)) -> p
            NoArg (Pus (p,_)) -> p
    in Left [emsgLC lc "" $ expType i 1 ty $$ etext "But lambda expression has pi type"]
typeOfH i _ _ j@(Idp _) (T exp@(Spi x _ a _)) = do
...
typeOfH i mctx ctx e@(Coe (PCoe (lc,_))) (T ty@(Spi v fv a@(Sid (Stype _) x y) b)) = case b 0 [] $ svar (freshName v fv) a of
...
typeOfH i mctx ctx e@(Pmap (Ppmap (lc,_))) (T exp@(Spi v a@(Sid (Spi v' a' b') f g) b)) =
    case isArr i a b of
        Just (Spi v2 a2'@(Sid a2 x y) b2') | cmpTypes a2 a' ->
...
typeOfH i mctx ctx (Ext (PExt (lc,_))) (T ty@(Spi x' s@(Spi _ _ a' b') t)) = case isArr i s t of
    Just (Sid (Spi _ a b) f g) | cmpTypes a a' ->
        let v = svar i a
        in if cmpTypes (b' 0 [] v) (Sid (b 0 [] v) (app 0 f v) (app 0 g v)) then Right ty else extErrorMsg i lc ty
...
typeOfH i mctx ctx (Ext (PExt (lc,_))) (T ty@(Spi x' s@(Ssigma _ a' b') t)) = case isArr i s t of
    Just (Sid (Ssigma x a b) p q) ->
        ...
        in if cmpTypes a' (Sid a (proj1 p) (proj1 q)) &&
              cmpTypes (b' 0 [] v) (Sid (b 0 [] $ proj1 q) (trans 0 (Slam x b) s $ proj2 p) (proj2 q))
           then Right ty
...
typeOfH i mctx ctx (Pair e1 e2) (T r@(Ssigma _ _ a b)) = do
...
typeOfH i mctx ctx (Proj1 (PProjl (lc,_))) (T r@(Spi x a'@(Ssigma _ _ a _) b)) = case isArr i a' b of
...
typeOfH i mctx ctx (Proj2 (PProjr (lc,_))) (T r@(Spi x a'@(Ssigma _ _ a b) b')) =
    if cmpTypes (b 0 [] $ liftTerm (\l -> T.App T.Proj1 $ T.LVar $ l - i - 1) a) (b' 0 [] $ svar i a')
        then Right r
...
typeOfH i mctx ctx (Inv _) (T exp@(Spi v a@(Sid t x y) b))
    | Just (Sid t' x' y') <- isArr i a b, cmpTypes t t' && cmpValues x y' t && cmpValues x' y t = Right exp
...
typeOfH i mctx ctx (App e1 e) N | Proj2 (PProjr p) <- dropParens e1 = do
    t <- typeOf i mctx ctx e
    case t of
        Ssigma _ _ a b -> Right $ b 0 [] $ evalRaw i mctx ctx (App (Proj1 $ PProjl p) e) (Just a)
        _ -> expTypeBut i "Sigma" e t
...
typeOfH i mctx ctx (Id a b) N = do
    ...
    return $ typeOfTerm i mctx ctx (reifyType a')
...
typeOfH i mctx ctx (Var (Arg (PIdent (lc,x)))) N = case M.lookup x ctx of
...
typeOfPmap :: T.DBIndex -> M.Map String DBIndex -> Ctx -> Value -> Value -> Value -> Value -> Expr -> Expr -> EDocM Value
typeOfPmap i mctx ctx (Spi v a b) f g (Sid a' x y) _ e2
    | cmpTypes a' a = Right $ Sid (b 0 [] y) (trans 0 (Slam v b) (evalRaw i mctx ctx e2 Nothing) (app 0 f x)) (app 0 g y)
...
isArr i t f =
    let r = f 0 [] (svar i t)
    in if elem i (valueFreeLVars i r) then Nothing else Just r

evalDecl :: T.DBIndex -> M.Map String DBIndex -> String -> Expr -> Maybe Expr -> StateT Ctx EDocM (Value,Value)
