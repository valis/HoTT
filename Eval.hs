...
eval 0 ctx (Pi ((vars,t):ts) e) = go ctx vars
  where
    ...
    go ctx []     = eval 0 ctx t `sarr` (eval 0 ctx (Pi ts e),efv)
...
eval 0 ctx (Id e1 e2) = Sid (eval 0 ctx e1) (eval 0 ctx e2)
...
eval _ _ (Id _ _) = error "TODO: eval.Id > 0"
...
rec 0 p z s = go
  where
    ...
    go t@(Ne e) = Ne $ Rec `aApp` reify p `aApp` reify z `aApp` reify s `aApp` e
    go _ = error "rec.go"
