...
\UnaryInfC{$\Gamma \vdash pcon\ p\ p'\ t : p = p' \,|\, Id_1\,(idp\ A)\,a\,a'$}
\DisplayProof
\end{center}

\medskip
\begin{center}
\AxiomC{$\Gamma \vdash t : p = p' \,|\, Id_1\,(idp\ A)\,a\,a'$}
...
\[ \D = [Hom(\theta)_\bot \times \D \to \D] \oplus \D \times \D \oplus (\omega + 1)_\bot \oplus \mathsf{List}(\D \times \D) \times \D \]
\[ \oplus \D \times \D \times TM_\bot \times \nats_\bot \oplus \D \times \D \times \D \oplus \nats_\bot \times \D \times \D \times \D \]
\[ \oplus \nats_\bot \times \D \times [Hom(\theta)_\bot \times \D \to \D] \]
\[ \oplus \nats_\bot \times \D \times [Hom(\theta)_\bot \times \D \to \D], \]
...
\item[] $\mathsf{Id} : \nats_\bot \times \D \times \D \times \D \to \D$
\item[] $\mathsf{Pi} : \nats_\bot \times \D \times [Hom(\theta)_\bot \times \D \to \D] \to \D$
\item[] $\mathsf{Sigma} : \nats_\bot \times \D \times [Hom(\theta)_\bot \times \D \to \D] \to \D$
\end{itemize}
...
\begin{comment}
\item[] $\mathsf{comp} : Ob(\theta)_\bot \times \D \times \D \to \D$
% \item[] $\mathsf{comp}(n, \mathsf{Idp}(x, y, d), \mathsf{Idp}(x', y', e)) = \mathsf{Idp}(\mathsf{comp}(n - 1, d, e))$, if $n > 0$
\item[] $\mathsf{comp}(n, \mathsf{Lam}(f), \mathsf{Lam}(f')) = $ \\
    \indent $\mathsf{Lam}((m,a) \mapsto \mathsf{comp}(dom(m), f(m, \delta^{dom(m)}_0 \cdot \tau^0_{dom(m)} \cdot a), f'(m, a)))$
\item[] $\mathsf{comp}(n, \mathsf{Pair}(a,b), \mathsf{Pair}(a',b')) = \mathsf{Pair}(\mathsf{comp}(a,a'), \mathsf{comp}(b,b'))$
\item[] $\mathsf{comp}(n, \mathsf{Path}(s,x), \mathsf{Path}(s',x')) = \mathsf{Path}(TODO)$
\item[] $\mathsf{comp}(n, \mathsf{Sigma}(k,A,B), \mathsf{Sigma}(k',A',B')) = \mathsf{Sigma}(TODO)$
\item[] $\mathsf{comp}(n, \mathsf{Pi}(k,A,B), \mathsf{Pi}(k',A',B')) = \mathsf{Pi}(TODO)$
\item[] $\mathsf{comp}(n, \mathsf{Id}(k,A,a,a'), \mathsf{Id}(k',B,b,b')) = \mathsf{Id}(TODO)$
\item[] $\mathsf{comp}(n, \mathsf{Ne}(l,r,t,c), \mathsf{Ne}(l',r',t',c')) = \mathsf{Ne}(TODO)$
\item[] $\mathsf{comp}(n, \mathsf{U}(\alpha), \mathsf{U}(\beta)) = \mathsf{U}(max(\alpha,\beta))$
\item[] $\mathsf{comp}(n, x, x') = \bot$, otherwise
\end{comment}
