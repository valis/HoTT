...
\begin{center}
\AxiomC{$\Gamma \vdash A : Type_{n+1}$}
...
\AxiomC{$\Gamma \vdash {\partial^\alpha_0}^* a : {\partial^\alpha_0}^* A$ for $\alpha = \pm$}
...
\medskip
\begin{center}
\AxiomC{$\Gamma_{-1} \vdash p : Id\ A\ a\ a'$}
\UnaryInfC{$\Gamma \vdash path^{-1}\ a\ a'\ p : A$}
\DisplayProof
\end{center}
