...
\medskip
\begin{center}
\AxiomC{$\Gamma \vdash A : A_0 = A_1\,|\,idp\ U_\alpha$}
\AxiomC{$\Gamma, A, A, v_1 = v_0\,|\,idp\ A \vdash\ \Uparrow^2_1\hspace{-.4em} B :\ \Uparrow^2\hspace{-.5em} B_0 =\ \Uparrow^1_1 \Uparrow^1\hspace{-.5em} B_1\,|\,idp\ U_\beta$}
\BinaryInfC{$\Gamma \vdash \Pi_1 A B : \Pi A_0 B_0 = \Pi A_1 B_1\,|\,idp\ U_{max(\alpha, \beta)}$}
\DisplayProof
\end{center}
...
\[ \D = [Hom(\bbG)_\bot \times \D \to \D] \oplus \D \times \D \oplus (\omega + 1)_\bot \oplus \D \times \D \times \D \]
...
\item[] $\mathsf{Idp} : \D \times \D \times \D \to \D$
...
\item[] $\mathsf{action}(\delta_n, e) = \mathsf{Idp}(e, e, e)$
\item[]
...
Let $\mathscr{M}_n$ denote the set of functions $(\D \times \D)^n \to \mathscr{P}(\D)$.
If $n \in \nats$, $R \in \mathscr{M}_n$, and $Q : \D \to \mathscr{M}_n$, then define $\mathcal{S}igma_n R\,Q \in \mathscr{M}_n$ as follows:
\[ \mathcal{S}igma_n R\,Q((\mathsf{Pair}(a_1,b_1), \mathsf{Pair}(a'_1,b'_1)), \ldots (\mathsf{Pair}(a_n,b_n), \mathsf{Pair}(a'_n,b'_n))) = \]
\[ \{ \mathsf{Pair}(a,b)\,|\,a \in R((a_1,a'_1), \ldots (a_n,a'_n)), b \in Q(a)((b_1,b'_1), \ldots (b_n,b'_n)) \}. \]
Let $\mathcal{S}igma_n R\,Q(p_1, \ldots p_n)$ be empty if arguments do not have the form as above.

Suppose that $n \in \nats$, $Q : \D \to \mathscr{M}_n$, and $R = (R_0, \ldots R_n)$ is a sequence such that $R_i \in \mathscr{M}_i$.
Then let $\mathcal{P}i_n R\,Q((\mathsf{Lam}(f_1),\mathsf{Lam}(f'_1)), \ldots (\mathsf{Lam}(f_n),\mathsf{Lam}(f'_n)))$
be equal to the subset of $\D$ of the form $\mathsf{Lam}(f)$ such that
\[ f(id_n,a) \in Q(a)((f_1(id_0,a_1), f'_1(id_0,a'_1)), \ldots (f_n(id_{n-1},a_n), f'_n(id_{n-1},a'_n))) \]
for all $a_1,a'_1 \in R_0, a_2,a'_2 \in R_1(a_1,a'_1), \ldots a_n,a'_n \in R_{n-1}(a_1,a'_1) \ldots (a_{n-1},a'_{n-1})$, and $a \in R_n((a_1,a'_1), \ldots (a_n,a'_n))$.
Let $\mathcal{P}i_n R\,Q(f_1, \ldots f_n)$ be empty if arguments do not have the form as above.

% \[ \mathcal{N}\!e = \{ \mathsf{Ne(s,e)}\,|\,s : \nats_\bot \to \D \times \D, e \in TM \}, \]
% \[ \mathcal{I}d\ A = \mathcal{N}\!e \cup \{ \mathsf{Idp}(d)\,|\,d \in A \}. \]

Now, for each $\alpha \leq \omega$ and $n \in \nats$,
we define $\mathcal{U}^\alpha_n \in \mathscr{M}_n$ and a function $[-]^\alpha_n : \mathcal{U}^\alpha_n \to \mathscr{M}_n$ by induction on $\alpha$.
First, we define a relation $G^\alpha_n \subseteq \D \times \mathscr{M}_n$ inductively by the following rules:
\medskip
\begin{center}
\AxiomC{$(A, R) \in G^\alpha_n$}
\AxiomC{$(B(id_n, a), Q(a)) \in G^\alpha_n$}
\BinaryInfC{$(\mathsf{Sigma}(n, A, B), \mathcal{S}igma_n R\,Q) \in G^\alpha_n$}
\DisplayProof
\end{center}
\medskip

\begin{center}
\AxiomC{$(A, R_i) \in G^\alpha_i$}
\AxiomC{$(B(id_n, a), Q(a)) \in G^\alpha_n$ for all $a \in R_n$}
\BinaryInfC{$(\mathsf{Pi}(n, A, B), \mathcal{P}i_n (R_0, \ldots R_n)\,Q) \in G^\alpha_n$}
\DisplayProof
\end{center}
\medskip
...
\begin{center}
\AxiomC{}
\UnaryInfC{$(\mathsf{Ne}(s, e), \mathcal{N}\!e\,s) \in G^\alpha_n$}
\DisplayProof
\quad
\AxiomC{}
...
We inductively define $\mathcal{U}^\alpha_n \in \mathscr{M}_n$ by the following rules:
\medskip
\begin{center}
...
\begin{center}
\AxiomC{}
\UnaryInfC{$\mathsf{Ne}(s,e) \in \mathcal{U}^\alpha$}
\DisplayProof
\quad
\AxiomC{}
\RightLabel{, $\beta < \alpha$}
\UnaryInfC{$\mathsf{U}(\beta) \in \mathcal{U}^\alpha$}
\DisplayProof
\quad
\AxiomC{}
\UnaryInfC{$\mathsf{Id}(0, a, x, y) \in \mathcal{U}^\alpha$}
\DisplayProof
\quad
\end{center}
...
\begin{lem}
For each $\alpha \leq \omega$, the graph $G^\alpha$ defines a partial function, that is $(a, A), (a, A') \in G^\alpha$ implies $A = A'$.
For each $\alpha \leq \omega$, the partial function $[-]^\alpha : \D \to \mathscr{P}(\D)$ is defined on $\mathcal{U}^\alpha \subseteq \D$.
\end{lem}
\begin{proof}
...
\item[] $\mathsf{reify}(i, \mathsf{Idp}(x, y, a), \mathsf{Id}(0, t, a', a'')) = idp\ \mathsf{reify}(i, a, t)$
\item[] $\mathsf{reify}(i, \mathsf{Ne}(s, e), t) = e(i)$
