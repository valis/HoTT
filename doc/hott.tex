...
Later, we will extend syntax by adding path types, universes, and (higher) inductive types.

Let $Vars$ be a countably infinite set of variables.
Then we define the sets of raw terms and contexts as follows:
\[ Tm ::= v\ |\ \Pi A\,B\ |\ \lambda v. b\ |\ f\ a\ |\ \Sigma A\,B\ |\ a , b\ |\ proj_1\ p\ |\ proj_2\ p\ |\ \alpha \cdot a, \]
\[ Ctx ::= \varnothing\ |\ \Gamma, A, \]
where $v \in Vars$, $\Gamma \in Ctx$, $a, b, f, p, A, B \in Tm$, and $\alpha$ is a morphism in the category $\Box$.
...
\RightLabel{, $\mathsf{reify}(|\Gamma|, 0, \llbracket A \rrbracket^{\rho_\Gamma}_0, \mathsf{U}(\omega)) = \mathsf{reify}(|\Gamma|, 0, \llbracket B \rrbracket^{\rho_\Gamma}_0, \mathsf{U}(\omega))$}
